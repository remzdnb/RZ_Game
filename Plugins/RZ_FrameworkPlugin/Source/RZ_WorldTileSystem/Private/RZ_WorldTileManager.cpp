...
ARZ_WorldTileManager::ARZ_WorldTileManager()
{
...
void ARZ_WorldTileManager::OnConstruction(const FTransform& Transform)
{
	Super::OnConstruction(Transform);
}

void ARZ_WorldTileManager::PostInitializeComponents()
{
	Super::PostInitializeComponents();
}

void ARZ_WorldTileManager::BeginPlay()
{
	Super::BeginPlay();
...
void ARZ_WorldTileManager::Tick(float DeltaTime)
{
...
void ARZ_WorldTileManager::AddSelection(const FVector& Location, FIntPoint Size)
{
	if (!PooledTiles.IsValidIndex(0))
	{
		UE_LOG(LogTemp, Error, TEXT("ARZ_WorldTileManager::EnableTile : Out of pooled tiles !"));
		return;
	}
	...
	for (int32 XIndex = FinalSize.X / 2 * -1; XIndex <= FinalSize.X / 2; XIndex++)
	{
		for (int32 YIndex = FinalSize.Y / 2 * -1; YIndex <= FinalSize.Y / 2; YIndex++)
		{
			...
			else
			{
				...
				if (bHasOddSize)
				{ //100.0f worldtilesize
					const FVector TargetTileLocation = FVector(
						Location.X + XIndex * 100.0f,
						Location.Y + YIndex * 100.0f,
						0.0f
					);
					...
					if (IsValidNewTileLocation(TargetTileLocation))
					{
						...
						PooledTiles[0]->Enable(FRZ_WorldTileData());
				...
				else
				{
					...
					if (IsValidNewTileLocation(TargetTileLocation))
					{
						PooledTiles[0]->SetActorLocation(FVector(
							Location.X + XIndex * 100.0f - 100.0f / 2 * PositiveX,
							Location.Y + YIndex * 100.0f - 100.0f / 2 * PositiveY,
	...
	for (int32 XIndex = IntRect.Min.X; XIndex < IntRect.Max.X; XIndex++)
	{
		for (int32 YIndex = IntRect.Min.Y; YIndex < IntRect.Min.Y; YIndex++)
		{
			...
			DisabledTiles[0]->Enable(FRZ_WorldTileData());
			...
			//SpawnTile(SpawnTransform, XIndex, YIndex, WORLDTILESIZE / 2, WORLDTILESIZE / 2, ERZ_WorldTileType::Grid);
		}
	}*/
	...
	if (bDebug)
	{
		UE_LOG(LogTemp, Display, TEXT("ARZ_WorldTileManager::AddRectSelection"));
	}
}

void ARZ_WorldTileManager::EnableTile(FIntPoint Position)
{
	if (!PooledTiles.IsValidIndex(0))
	{
		UE_LOG(LogTemp, Error, TEXT("ARZ_WorldTileManager::EnableTile : Out of pooled tiles !"));
	...
	if (bDebug)
	{
		UE_LOG(LogTemp, Display, TEXT("ARZ_WorldTileManager::EnableTile"));
	}
}

void ARZ_WorldTileManager::AddRectSelection(FRZ_IntRect Rect)
{
	FIntPoint Index;
...
void ARZ_WorldTileManager::ClearActiveTiles()
{
...
bool ARZ_WorldTileManager::IsValidNewTileLocation(const FVector& TargetLocation) const
{
...
bool ARZ_WorldTileManager::IsActiveTileLocation(const FVector& TargetLocation) const
{
	for (const auto& ActiveTile : ActiveTiles)
	{
...
void ARZ_WorldTileManager::Debug(float DeltaTime)
{
	if (!bDebug) { return; }
	...
	const FString FinalString = " ------------------- ARZ_WorldTileManager::Debug ------------------- ";
	GEngine->AddOnScreenDebugMessage(-1, DeltaTime, FColor::Purple, FinalString);;
}

/*
void ARZ_WorldTileManager::SpawnTile(const FTransform& SpawnTransform, int32 PositionX,
